...
void view_transaction(const char *acc);
...
int main()
{
    ...
    do
    {
        ...
        p_line("  1 => Create Account\n", 0.1);
        p_line("  2 => Log-In Account\n", 0.1);
        p_line("  3 => Delete Account\n", 0.1);
        p_line("  4 => Exit\n", 0.1);
        ...
        {
            p_line("\nInvalid Input!\n", 0.4);
            p_line("SORRY! We're Exiting", 0.4);
            p_line("-------\n", 1.5);
            clear_screen();
            return 1;
        }
...
// TO LOGIN USER AND ACCOUNT DETAILS
void handle_login_acc(void)
{
